...
const WinMultiplierEffect: React.FC<WinMultiplierEffectProps> = ({
  ...
  const { freeSpinCombination, isLoading } = useSlotMachineOdds();
  ...
  useEffect(() => {
    ...
    // Create initial effects with staggered delays
  ...
  return (
    ...
    >
      ...
      <div className="relative text-center">
        {/* Win label with enhanced text styling */}
        <motion.div
        ...
        {/* Multiplier with enhanced 3D text effect */}
        {multiplier >= 1 && (
          <motion.div
        ...
        {/* Win amount with glowing effect */}
        <motion.div
          ...
          animate={{
            ...
            y: 0,
            filter: [
              `drop-shadow(0 0 2px ${tier.color})`,
              `drop-shadow(0 0 8px ${tier.color})`,
              `drop-shadow(0 0 2px ${tier.color})`,
            ],
          }}
          transition={{
            delay: effect.delay + 0.3,
            duration: 0.5,
            filter: {
              duration: 2,
              repeat: Infinity,
      ...
      {/* Floating particles with enhanced randomization */}
...
const ParticleField: React.FC<ParticleFieldProps> = ({
  ...
  const particleCount = Math.min(16, 6 + Math.floor(Math.abs(multiplier) / 2));

  return (
    <>
      {Array.from({ length: particleCount }).map((_, i) => {
        // Create a spiral or circular pattern for particles
        const angle = (Math.PI * 2 * i) / particleCount;
        const radius = 80 + Math.random() * 60;

        // Vary particle sizes
        const size = 3 + Math.random() * (multiplier > 5 ? 6 : 4);
        ...
        return (
          <motion.div
            ...
            animate={{
              ...
              rotate: [0, 360],
              boxShadow: [
                `0 0 ${size}px ${tier.shadowColor}`,
                `0 0 ${size * 4}px ${tier.shadowColor}`,
                `0 0 ${size}px ${tier.shadowColor}`,
              ],
            }}
            transition={{
